...
# Uses the GitLab and GitHub APIs to migrate repositories GL->GHEC
...
GITLAB_ENDPOINT = "https://git.corporation.net/api/v4"
...
def get_matching_gitlab_projects(gl_token:str, pattern:str) -> list:
    headers = {
        'Accept': 'application/json',
         'Authorization': f'Bearer {gl_token}'
    }
    print(f"Searching for regex pattern: '{args.regex}' within GITLAB...")
    projects_paths = []
    clone_paths = []
    page=1
    while True:
        print(f"Matching repos - page {page} ...")
        response = requests.get(f'{GITLAB_ENDPOINT}/projects', headers=headers, params={'page': page, 'per_page': 100})
        data = response.json()
        ...
        # Get the json element 'path_with_namespace' which contains the GitLab Project path of the repo
        # And cross match with the regex provided
        list_paths = [project['path_with_namespace'] for project in data if re.search(pattern=pattern, string=project['path_with_namespace'])]
        list_paths = ["/".join(repo.split("/")[-2:]) for repo in list_paths]

        list_clones = [project['http_url_to_repo'] for project in data if re.search(pattern=pattern, string=project['path_with_namespace'])]
        projects_paths.extend(list_paths)
        print(f"Matching projects so far: {projects_paths}")
        clone_paths.extend(list_clones)
        # print(f"DEBUG: Clone {clone_paths}")
        page += 1
        # break  ## COMMENT OR UNCOMMENT TO SEARCH ONLY ONE PAGE

    combined_list = list(zip(projects_paths, clone_paths))
...
if ( __name__ == "__main__"):
    ...
    parser.add_argument('-gp','--github-pem', help="GitHub App PEM key path", required=False, default='./gh_app.pem')
    ...
    gitlab_repos_list = get_matching_gitlab_projects(gl_token=gl_token,pattern=repo_regex)
